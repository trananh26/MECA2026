...
| `CMx` | Nhận | Quay băng tải CV02 | Ghi **M701 = 1**, chờ **M2302 = 1** rồi gửi **COx** |
| `C2x` | Nhận | Quay ngược CV03_IP02 | Ghi **M703 = 1** |
| `COx` | Gửi | Xác nhận CV02_IO01 có hàng | Gửi khi **M2302 = 1** (CV02_IO01) |
...
sequenceDiagram
    ...
    Ext->>SWM: CMx
    SWM->>PLC: M701 = 1
    Note over PLC: Băng tải quay, hàng tới CV02_IO01
    PLC->>SWM: M2302 = 1
    SWM->>Ext: COx
```
...
sequenceDiagram
    ...
    Note over Ext,SWM: IP01 có hàng (M2300=1)
    Ext->>SWM: C1x
    SWM->>SWM: Tạo lệnh AGV IP01 → BF
```

**Lưu ý:** **C1** và **CM** là hai lệnh độc lập — `CM` chỉ quay băng tải, `C1` chỉ tạo lệnh AGV nhập kho.

Điều kiện tạo lệnh (`CreateImportCommand`):

1. PLC **M2300 = 1** (IP01 có hàng).
...
| Tag | Ý nghĩa |
|-----|---------|
| `M2300` | IP01 có hàng (=1) / trống (=0) |
| `M2302` | CV02_IO01 có hàng (=1) — điều kiện gửi COx |
| `M2301` | OP01 có hàng (=1) / trống (=0) |
| `M510` | AGV có hàng (=1 FULL) / trống (=0 EMPTY) |
| `D800` | Vị trí AGV thô; nếu > 4 thì trừ 4 để map node trên sơ đồ |
| `M3000` | Hoàn thành lệnh (=1) |
...
| Tag | Ý nghĩa |
|-----|---------|
| `M701` | Quay băng tải CV02 (=1 khi nhận CMx) |
| `M703` | Quay ngược băng tải CV03_IP02 (=1 khi nhận C2x) |
| `M39` | Cờ gửi lệnh AGV (0 → ghi D502/D500 → 1) |
...
| `TRANSFERING DEST` | AGV tới node đích **hoặc** M3000=1 → `JOB COMPLETE`, đích → FULL |
...
| File | Trách nhiệm |
|------|-------------|
| `PlcService.cs` | Kết nối ActUtlType, đọc/ghi tag, `ApplyJobToPlc`, `StartConveyorRotation` |
| `ConveyorCommandService.cs` | CMx → M701, M2302 → COx; C2x → M703 |
...
| Gửi CMx | M701=1, chờ M2302=1 rồi nhận COx |
...
[Thiết bị ngoài gửi CMx]
  ...
  SCADA set M701=1
      │
      ▼
  Chờ M2302=1 (CV02_IO01 có hàng) ──► gửi COx
      │
      │ (băng tải đưa hàng tới IP01, M2300=1)
      ▼
...
## 12. Thay đổi / mở rộng thường gặp
